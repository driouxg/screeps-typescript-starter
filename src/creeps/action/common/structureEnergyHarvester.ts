...
export default class StructureEnergyCollector implements ICreepEnergyRetrieval {
  retrieve(creep: Creep): void {
    // storage
    if (creep.room.storage && 0 < creep.room.storage.store.getFreeCapacity(RESOURCE_ENERGY)) {
    ...
    // extensions
    const extensions = findContainers(creep.room).filter(e => 0 < e.store.getFreeCapacity(RESOURCE_ENERGY));
